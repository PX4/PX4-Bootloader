...
*/
uint32_t code_vectors[2*1024/4];    // 2K  , 中断向量表
uint32_t code_encryption[2*1024/4]; // 2K  , 加密代码
...
void erase_code(void)
{
	...
	for (i = 0; i < len; i++) {
		// program the word
		flash_program_word(address + FLASH_START_ADDRESS, code_vectors[i]);
		address += 4;
	}
	address=0x3800;  // code_encryption
	for (i = 0; i < len; i++) {
		// program the word
		flash_program_word(address + FLASH_START_ADDRESS, code_encryption[i]);
		address += 4;
	}
	//main();
#if 1
...
void __attribute__ ((section(".reset_e"))) main_e(void)
{
	...
	code_vectors[1] = (uint32_t)reset_handler_m;
	erase_code(); // 写入操作,擦除原有代码

	// 执行系统复位
	scb_reset_system(); 
}
void __attribute__ ((section(".reset_e"))) reset_handler_rb1(void)
{
	//volatile unsigned *src, *dest;
	funcp_t *fp;
	reset_handler_init();
...
void __attribute__ ((section(".reset_b2"))) reset_handler_rb2(void)
{
	funcp_t *fp;
	reset_handler_init();
