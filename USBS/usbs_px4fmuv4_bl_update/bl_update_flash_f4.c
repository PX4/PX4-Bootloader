...
#if 0
void
flash_func_erase_code(unsigned sector)
{
	if (sector >= BOARD_FLASH_SECTORS || sector < 0) {
		return;
	}
...
#endif
#define UID0    (0x35dc24eaU)
#define UID1    (0x4fd84b68U)
#define UID2    (0xa8345ed9U)
...
//extern uint16_t bl_flash_get(char code[], const uint16_t _size, const uint8_t set_head);
...
int main(void)
{
	...
	uint32_t i=0;
	//const uint32_t* src= (const uint32_t*)0x08008000;
	...
	i=0;
	while (1) {
		if(i<10)uart_cout((uint8_t *)"Erase\r\n", 7);
		i++;
